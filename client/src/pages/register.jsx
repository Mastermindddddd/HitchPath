...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(result.errors?.map((err) => err.msg).join(", ") || result.error || "Registration failed.");
        setSuccessMessage("");  // Clear any success message if there is an error
      }
    } catch (error) {
  ...
  return (
    <div class="font-[sans-serif] flex md:h-screen p-4">
      <div class="w-full max-w-4xl max-md:max-w-xl mx-auto">
        <div class="bg-white grid md:grid-cols-2 w-full sm:p-8 p-6 shadow-md rounded-md overflow-hidden">
          <div class="max-md:order-1 space-y-6">
            <div class="md:mb-16 mb-8">
              <h3 class="text-gray-800 text-2xl font-bold"></h3>
            </div>
          ...
          <form className="w-full" onSubmit={handleSubmit}>
            ...
            {successMessage && (
                <div className="text-green-600 bg-green-100 p-3 rounded-md border border-green-400 mb-4">
                  {successMessage}
                </div>
              )}

              {error && (
                <div className="text-red-600 bg-red-100 p-3 rounded-md border border-red-400 mb-4">
                  {error}
                </div>
              )}
